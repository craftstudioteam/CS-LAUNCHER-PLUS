        ...
        sExecutorService.execute(() -> {
            try {
                ...
                if (doesOwnGame) {
                    ...
                    // Phase 9: cache the FULL premium skin (Mojang textures by
                    // UUID) right at login, so the home 3D character and the 3D
                    // head show the real skin on the very first frame — the
                    // in-game skin was always right, the launcher just never
                    // had the sheet on disk.
                    try {
                        acc.save(); // resolver reads the saved account kind
                        java.io.File full = net.kdt.pojavlaunch.skins.SkinResolver.resolve(acc);
                        if (full != null) net.kdt.pojavlaunch.ui.SkinHead3DRenderer.invalidate(acc.username);
