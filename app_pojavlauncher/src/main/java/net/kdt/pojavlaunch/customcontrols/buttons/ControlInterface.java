...
public interface ControlInterface extends View.OnLongClickListener, GrabListener {
    ...
    default void injectTouchEventBehavior() {
        getControlView().setOnTouchListener(new View.OnTouchListener() {
            ...
            private boolean mDragArmed;
            private float mRestAlpha = 1f;
            ...
            public boolean onTouch(View view, MotionEvent event) {
                ...
                switch (event.getActionMasked()) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        if (!mDragArmed && event.getEventTime() - downTime >= 220) {
                            mDragArmed = true;
                            mCanTriggerLongClick = false;
                            view.performHapticFeedback(android.view.HapticFeedbackConstants.LONG_PRESS);
                            // ── PICK-UP: the control lifts off the canvas — scales up,
                            // goes translucent and floats above its neighbours.
                            mRestAlpha = view.getAlpha();
                            view.animate().cancel();
                            view.animate()
                                    .scaleX(1.09f).scaleY(1.09f)
                                    .alpha(Math.min(1f, mRestAlpha) * 0.85f)
                                    .translationZ(12f)
                                    .setDuration(160)
                        ...
                        float targetX = event.getRawX() - parentLocation[0] - touchOffsetX;
                        float targetY = event.getRawY() - parentLocation[1] - touchOffsetY;
                        ...
                        float clampedX = MathUtils.clamp(targetX, 0, maxX);
                        float clampedY = MathUtils.clamp(targetY, 0, maxY);
                        // Smart guides: magnetic lock FIRST (so the committed
                        // position and the drawn guides always agree), then the
                        // neighbour snapping, then the paint-only guide pass.
                        float[] aligned = parent.alignDrag(view, clampedX, clampedY);
                        snapAndAlign(MathUtils.clamp(aligned[0], 0, maxX),
                                MathUtils.clamp(aligned[1], 0, maxY));
                        parent.updateDragGuides(view);
                        break;

                    case MotionEvent.ACTION_UP:
                        if (!mDragArmed
                                && Math.abs(event.getRawX() - downRawX) <= mTouchSlop
                                && Math.abs(event.getRawY() - downRawY) <= mTouchSlop) {
                            onLongClick(view);
                        }
                        if (mDragArmed) settleAfterDrag(view, mRestAlpha);
                        getControlLayoutParent().clearDragGuides();
                        mDragArmed = false;
                        break;
    ...
    /** ── DROP: springy landing after a drag — the control settles with a
     *  soft overshoot back to its resting scale/opacity. */
    default void settleAfterDrag(View view, float restAlpha) {
        view.animate().cancel();
        view.animate()
                .scaleX(1f).scaleY(1f)
                .alpha(restAlpha)
                ...
                .withEndAction(() -> {
                    ...
                    view.setAlpha(restAlpha); view.setTranslationZ(0f);
                })
                .start();
