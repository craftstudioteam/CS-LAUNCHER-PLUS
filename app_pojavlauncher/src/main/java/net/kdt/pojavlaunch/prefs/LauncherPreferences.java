...
public class LauncherPreferences {
    ...
    /** Settings the launcher used to act on and no longer reads. */
    private static final String[] OBSOLETE_PERFORMANCE_KEYS = {
            "bigCoreAffinity", "cs_perf_boost", "cs_perf_big_cores", "thermalTelemetry", "low_pause_gc",
            // A stored true here used to mean "hold the display at its sustained rate" — on many panels
            // that is a 60 FPS ceiling, so the key is retired rather than inherited.
    ...
    public static String PREF_GAME_LOG_DISPLAY_MODE = "animated_smart";
    ...
    public static void loadPreferences(Context ctx) {
        ...
        // The launcher and the game are two processes with two independent copies of this file.
        // Pull anything the other one changed back in BEFORE the values below are read, otherwise
        // this process would keep serving (and later rewrite) a stale copy — that is what made the
        // in-game FPS toggle come back off after every relaunch.
        try {
            SharedSettings.syncIntoPreferences(ctx);
        } catch (Throwable e) {
            Log.w("LauncherPreferences", "cross-process settings sync failed", e);
        }
        ...
        // Removed hacks, deleted rather than left dormant: per-thread affinity is a no-op for an app
        // (Android confines the process through cpusets), and the boost switches that used to drive it
        // are now expressed by the performance profile. Dropping the keys stops an old install from
        // carrying a setting nothing reads.
        for (String obsoleteKey : OBSOLETE_PERFORMANCE_KEYS) {
            if (DEFAULT_PREF.contains(obsoleteKey)) {
                DEFAULT_PREF.edit().remove(obsoleteKey).apply();
            }
        }
        ...
        PREF_GAME_LOG_DISPLAY_MODE = DEFAULT_PREF.getString("game_log_display_mode", "animated_smart");
    ...
    public static void writeMGRendererSettings() throws IOException {
        ...
        try {
            String desired = Tools.GLOBAL_GSON.toJson(MGConfigJson);
            String existing = configFile.isFile() ? Tools.read(configFile.getAbsolutePath()) : null;
            if (!desired.equals(existing)) {
                Tools.write(configFile.getAbsolutePath(), desired);
                Logger.appendToLog("Updated MG configs at " + configFile.getAbsolutePath());
            }
        } catch (IOException e) {
