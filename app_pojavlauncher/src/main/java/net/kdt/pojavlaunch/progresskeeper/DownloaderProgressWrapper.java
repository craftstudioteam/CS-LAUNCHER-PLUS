...
public class DownloaderProgressWrapper implements Tools.DownloaderFeedback {
    ...
    /**
     * Segmented downloads call updateProgress on every 128 KB chunk, from every
     * parallel stream. Forwarding all of that to the main thread floods it with
     * ~80 posts/second — the UI stutters and the whole install feels slower.
     * The *values* below are still measured from every single chunk; we only
     * stop re-posting an identical number, except ~4×/second and always at 100%.
     */
    private long mLastSubmitTime = -1;
    private int mLastSubmittedPct = -1;
    ...
    public void updateProgress(int curr, int max) {
        ...
        boolean complete = max > 0 && curr >= max;
        if (!complete
                && percentage == mLastSubmittedPct
                && (now - mLastSubmitTime) < 250L) return;
        mLastSubmitTime = now;
        mLastSubmittedPct = percentage;
