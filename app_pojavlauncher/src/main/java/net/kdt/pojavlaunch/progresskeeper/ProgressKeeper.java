...
public class ProgressKeeper {
    ...
    /**
     * The last percentage a record reported, without handing out the state object.
     * The notification heartbeat reads this instead of relying on being called:
     * the number is always available even if the events that produced it were
     * coalesced away while the app sat in the background.
     *
     * @return 0..100, or -1 when the record is unknown/finished
     */
    public static synchronized int peekProgress(String progressRecord) {
        ProgressState state = sProgressStates.get(progressRecord);
        if (state == null) return -1;
        return state.progress;
    }

    /** Resource id behind the record's current status line, or -1. */
    public static synchronized int peekResid(String progressRecord) {
        ProgressState state = sProgressStates.get(progressRecord);
        return state == null ? -1 : state.resid;
    }

    /** Whether the record's last state still carries moving byte figures. */
    public static synchronized boolean peekHasLiveBytePayload(String progressRecord) {
        ProgressState state = sProgressStates.get(progressRecord);
        return state != null && state.hasLiveBytePayload();
    }

    /** Formatting arguments for the record's status line, or null. */
    public static synchronized Object[] peekVarArgs(String progressRecord) {
        ProgressState state = sProgressStates.get(progressRecord);
        return state == null ? null : state.varArg;
    }
